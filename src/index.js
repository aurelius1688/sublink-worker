  ...
  async fetch(request, env, ctx) {
    try {
      ...
      if (request.method === 'GET' && url.pathname === '/') {
        ...
        let baseConfig;
        if (configId) {
          const customConfig = await env.SUBLINK_KV.get(configId); // 使用 env 而不是全局变量
          if (customConfig) {
        ...
        await env.SUBLINK_KV.put(shortCode, originalUrl); // 使用 env
        ...
        await env.SUBLINK_KV.put(shortCode, queryString); // 使用 env
        ...
        const originalParam = await env.SUBLINK_KV.get(shortCode); // 使用 env
        let originalUrl;
        ...
        try {
          ...
          await env.SUBLINK_KV.put(configId, configString, { // 使用 env
